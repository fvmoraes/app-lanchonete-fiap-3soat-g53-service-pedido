...
public class RabbitMQConfig {
	...
	Binding pedidoBinding(Queue pagamentoQueue, DirectExchange pedidoExchange) {
		return BindingBuilder.bind(pagamentoQueue).to(pedidoExchange).with(PEDIDO_PAGAMENTO_ROUTING_KEY);
	}

	@Bean
	Queue producaoQueue() {
		return QueueBuilder.nonDurable(PRODUCAO_QUEUE_2).build();
	}

	@Bean
	Binding pedido2Binding(Queue producaoQueue, DirectExchange pedidoExchange) {
		return BindingBuilder.bind(producaoQueue).to(pedidoExchange).with(PEDIDO_PRODUCAO_ROUTING_KEY);
	}
